...
- Kubernetes cluster with Connectors system installed (Operator, Core and Git components). See the [Installation Guide](../install/install.mdx) for details on installing these components.
...
Example output:
```
[http "http://c-git-connector.git-connector-demo.svc"]
