...
metadata:
  name: oci-registry-demo
  annotations:
    cpaas.io/description: "Connect to CNCF Distribution Registry for team public repository access"
```

### Address

The `spec.address` specifies the access address of the OCI Registry, for example: `https://registry.example.com`.
...
## Proxy and Configuration \{#configuration}
...
Example:
registry.example.com/test/abc:v1 → c-oci-registry-demo.default.svc.cluster.local/namespaces/oci-connector-demo/connectors/oci-connector/test/abc:v1
...
**registry-config**: Configuration credentials required by OCI CLI like buildkit, buildah.
