...
- **`.curlrc`**: A `curl` config file containing the forward proxy URL with credentials, so curl-driven steps can opt in via `curl --config /{mount-path}/.curlrc` without exporting environment variables
...
>
> Alternatively, curl-driven Tasks can skip environment variables entirely by passing `--config /{mount-path}/.curlrc` directly to each curl invocation. For HTTPS destinations, also pass `--cacert /{mount-path}/ca.crt` because the forward proxy re-signs TLS traffic.
...
In the following example, we use a `buildah` Task that accepts a `registry-config` workspace with a `.env` file containing `http_proxy`, `https_proxy`, and `no_proxy` environment variables:
...
In the following example, we use a `buildah-cli` Task that accepts a `cmd` parameter for custom shell commands. The Task mounts the `registry-config` workspace to `/workspace/registry-config`:
