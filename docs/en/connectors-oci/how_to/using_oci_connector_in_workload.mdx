...
- To enable image pulls via the OCI Connector proxy in the Kubernetes runtime, you must expose the `oci` component's service using either NodePort or Ingress, configured through `spec.components.oci.spec.expose` on `ConnectorsConfig`. See [Expose the OCI Proxy Externally](../../install/install.mdx#oci-expose) for the full configuration.
...
- **Connectors system installed:** The Connectors Operator is installed and its `ConnectorsConfig` is reconciled to `Ready`. The `core` and `oci` components must be enabled. See the [Installation Guide](../../install/install.mdx) for details.
- **OCI component exposed externally (NodePort or Ingress):** Configure `spec.components.oci.spec.expose` on `ConnectorsConfig` to expose the OCI Connector proxy outside the cluster. See [Expose the OCI Proxy Externally](../../install/install.mdx#oci-expose).
