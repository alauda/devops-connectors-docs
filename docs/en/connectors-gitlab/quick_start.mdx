...
- Kubernetes cluster with Connectors system installed (ConnectorsCore and ConnectorsGitLab components). See the [Installation Guide](../install/install.mdx) for details on installing these components.
...
```
[http "http://c-gitlab-connector.gitlab-connector-demo.svc"]
...
This configuration:
- Adds an Authorization header with the encoded K8S API Token, applied only to requests sent to the proxy service address
