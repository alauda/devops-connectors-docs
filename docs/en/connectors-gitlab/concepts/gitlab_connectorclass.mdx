...
## Requirements for GitLab Servers and GitLab CLI (glab) \{#requirements-for-gitlab-servers-and-gitlab-cli-glab}
...
```ini
[http "http://c-gitlab-demo.default.svc"]
...
**How it works:**
1. The URL-scoped `[http "<proxy address>"]` section adds an Authorization header with a Kubernetes API server token (a temporary token used for requests to the GitLab connector proxy service); the header is sent only to the proxy address, so requests to other hosts (for example submodules hosted on a different git server) are not affected
