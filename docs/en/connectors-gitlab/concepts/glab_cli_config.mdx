...
========================================================
   GitLab instance (root)
        |
        +-- acme            <- tenantGroup (top-level)
        |     +-- backend   <- subgroup
        |     +-- frontend  <- subgroup
        |
        admin user PAT    -> creates `acme`; owner because creator
...
========================================================
   GitLab instance (root)
        |
        +-- umbrella                <- pre-existing umbrella group
        |     |
        |     +-- acme              <- tenantGroup (subgroup of umbrella)
        |     |     +-- backend
        |     |     +-- frontend
        |
        admin user PAT       -> user is direct Owner of `umbrella`,
                                so it can create subgroups beneath it
                                AND mint per-tenant GATs on those
                                subgroups (GAT-creation requires direct
                                Owner membership; Group Access Tokens
                                cannot satisfy this)
...
The lifecycle is depicted below:
