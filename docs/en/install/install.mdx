...
- **ConnectorsGit**: Optional component that adds support for generic Git services.
- **ConnectorsGitHub**: Optional component that adds support for GitHub-specific features.
...
- **ConnectorsSonarQube**: Optional component that adds support for SonarQube and SonarCloud platforms for code quality analysis.
...
### Install ConnectorsGitHub (Optional)

To add support for GitHub integration, install the ConnectorsGitHub component:

1. Create the ConnectorsGitHub custom resource:
...
3. Wait until the status shows that ConnectorsGitHub is ready:

   ```bash
   kubectl wait --for=condition=Ready connectorsgithub/connectors-github -n connectors-system --timeout=300s
   ```

4. Verify that the GitHub ConnectorClass has been created:
...
### Install ConnectorsNexus (Optional)

To add support for integration with Nexus repositories, install the ConnectorsNexus component:

1. Create the ConnectorsNexus custom resource:
...
3. Verify that the Nexus ConnectorClass is ready:
