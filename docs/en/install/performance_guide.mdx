...
The whole Connectors install is driven by a single `ConnectorsConfig` resource (see the [Installation Guide](./install.mdx)). To tune throughput, edit that same `ConnectorsConfig` and set workload settings under `spec.components.<name>.spec.workloads`.

Example `ConnectorsConfig` that tunes the `core` and `oci` components:
