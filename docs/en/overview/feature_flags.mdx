...
> **Note:** Direct modifications to the `connectors-config` ConfigMap (e.g., via `kubectl edit configmap`) are overwritten during the next reconciliation. Always use the `ConnectorsCore` CR to configure feature flags.
