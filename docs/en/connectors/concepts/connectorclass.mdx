...
`spec.addressExtensions` defines additional named addresses for the current tool type. It uses the same schema as `spec.params`:

- `spec.addressExtensions[].name` - Extension name. Must be unique in one ConnectorClass.
- `spec.addressExtensions[].type` - Value type. Currently only `string` is supported.
- `spec.addressExtensions[].default` - Default value (optional). If not set, this extension is required in Connector.
- `spec.addressExtensions[].description` - Description (optional).

This mechanism is commonly used when one connector needs multiple backend addresses, for example one Web access address and one API addres.
...
Connector admission follows the class contract defined here:

- Each extension without `default` must be provided by Connector.
- Connector cannot provide extension names that are not defined in `ConnectorClass.spec.addressExtensions`.

For connector-side values and runtime usage, refer to [Connector Address Extensions](./connector.mdx#address-extensions).
...
- `spec.authProbes[].probe.http.response.cel` specifies the expected response for authentication validation.
  - **Default behavior**: Returns 200 for success.
  - **Custom validation**: When configured with a CEL expression, authentication succeeds if the expression evaluates to `true`.

The CEL expression can use the following variables:
...
| `response.body`        | `object`              | Response body parsed as JSON           |
...
#### Variables Available in Rego \{#variables-available-in-rego}
...
When `x-provider-type: proxy` is used, you can also define `x-openapi-address` on the same operation to choose the backend address:

- empty or omitted: fallback to `Connector.spec.address`
- extension name: resolve from `Connector.spec.addressExtensions[*].name`
- direct URL: use a full `http/https` address
...
Configuration capabilities define the configuration files that a ConnectorClass provides to clients. These files can be mounted into Pods using the [Connectors-CSI Driver](./connectors_csi.mdx).

### Configuration Types

- **Custom Configurations**: Defined in `spec.configurations`
- **Built-in Configurations**: Automatically provided by Connectors-CSI Driver (see: [Built-in Configurations](./connectors_csi.mdx#built-in-configurations))
...
spec:
  ...
  - name: gitconfig
    data:
      .gitconfig: |
        [http "{{ .connector.status.proxyAddress }}"]
...
For more details, see: [Configuration File Rendering](./connectors_csi.mdx#configuration-file-rendering)
