...
To additionally require that specific people or groups approve, add `params`
(such as `approvers` and `numberOfApprovalsRequired`) to the check rule. See
[Restricting Who Can Approve](./connectors_approval.mdx#restricting-who-can-approve).
