...
2. Injects tool credentials specified in the Connector only when the request targets an allowed address of that Connector
...
#### Forward Proxy Credential-Injection Targets

For built-in forward proxy mode, credential injection targets are:

- `Connector.spec.address`
- `Connector.spec.addressExtensions[*].value`

When `addressExtensions` is not configured, only `spec.address` is eligible.
Changing these addresses requires permission to update the Connector itself, so this capability does not create a new credential ownership boundary.
...
### Connectors Proxy Authentication \{#connectors-proxy-authentication}
...
By default, when `enable-connector-proxy-permissions` is disabled, read permission on the `Connector` is enough to use `Connectors Proxy`.

When `enable-connector-proxy-permissions` is enabled, the system performs an additional `connectors/proxy` permission check for the target Connector. This is the extra capability permission that separates Connector discovery from real runtime use through the proxy path.

For the overall model and the meaning of `connectors/proxy`, see [Connectors Permission Model](./permission_model.mdx).
...
:::warning
- The `%2F` is the URL-encoded form of `/` in the connector namespace/name format.
- When specifying multiple connectors, separate them with commas. There is no strict limit on how many connectors you may list, but avoid configuring an excessive number to prevent oversized request headers and related issues.
:::
