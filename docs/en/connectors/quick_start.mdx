...
- This quick start assumes you already have the Connectors Operator installed and a `ConnectorsConfig` reconciled to `Ready` (the operator creates one automatically — see the [Installation Guide](../install/install.mdx)). The default install enables every component, including `core` and `git`, which are required for this guide.
...
- Kubernetes cluster with the Connectors Operator installed and a `ConnectorsConfig` reconciled to `Ready`. The default install enables the `core` and `git` components needed for this guide.
...
1. Check that the `ConnectorsConfig` is reconciled to `Ready`:

   ```bash
   kubectl get connectorsconfig -A
   ```

   `READY` should report `True`. For per-component phases, run `kubectl describe connectorsconfig connectors-config -n <namespace>`.
...
2. Verify that the Git ConnectorClass exists:
   ...
   If the Git ConnectorClass is not found, the `git` component is disabled or has not finished installing. Enable it on `ConnectorsConfig` (`spec.components.git.enabled: true`) and re-check — see the [Installation Guide](../install/install.mdx) for the configuration syntax.
...
1. **ConnectorClass Availability**: Ensure the Git ConnectorClass exists:
   ...
   kubectl get connectorclass git
   ```
   If it doesn't exist, the `git` component is disabled or has not finished installing on `ConnectorsConfig`. See the [Installation Guide](../install/install.mdx) for the configuration syntax.
