...
Set the flag declaratively on the cluster-singleton `ConnectorsConfig` so the value survives reconciliation. Do **not** edit the managed `connectors-config` ConfigMap directly — it is reconciled back from `ConnectorsConfig` on the next reconcile.

```bash
kubectl patch connectorsconfig connectors-config -n connectors-operator --type merge \
  -p '{"spec":{"components":{"core":{"spec":{"featureFlags":{"enable-custom-ca-certs":"true"}}}}}}'
```

Or edit `ConnectorsConfig` interactively:
...
For the full update syntax, see [Feature Flags](../../overview/feature_flags.mdx#update-a-feature-flag).
