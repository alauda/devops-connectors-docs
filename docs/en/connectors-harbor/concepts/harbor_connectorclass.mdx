...
#### Harbor CLI configuration \{#harbor-cli-configuration}

Use `harbor-cli-config` when you want a workload to run `harbor-cli` commands through the Harbor Connector without putting the original Harbor username or password into the Pod.

After you mount `harbor-cli-config`, your workload gets a ready-to-use Harbor CLI setup:

- `config.yaml` for Harbor CLI
- `.env` for the connector proxy environment
- `HARBOR_ENCRYPTION_KEY`, which Harbor CLI needs when reading the generated config

For a single Harbor Connector, mount it like this:
...
For multiple Harbor Connectors, request them in one mount. The generated `config.yaml` includes one Harbor CLI context for each selected Connector, so you can switch between them with `harbor context switch`.
...
::: info
The multiple connectors feature depends on `enable-multi-connector` feature flag. For flag details, see [Feature Flags](../../overview/feature_flags.mdx).
:::

Before running `harbor-cli`, load the proxy environment, trust the proxy CA, and export the mounted encryption key:
...
`HARBOR_ENCRYPTION_KEY` is only used so Harbor CLI can read the generated config. It is not the original Harbor secret, and the generated password value in `config.yaml` is not used as the real credential for Harbor access. Real access still goes through the connector proxy.
