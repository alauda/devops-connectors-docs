...
2. **Common Causes and Solutions**:
   ...
   **Scanner JRE Fails on Alauda OS Nodes**:
   - Error:
     ```text
     Error occurred during initialization of VM
     Failed to mark memory page as executable - check if grsecurity/PaX is enabled
     ```
   - Context: On Alauda OS or other hardened node images, newer SonarScanner CLI versions may auto-provision a JRE before running the scanner engine. If that downloaded JRE cannot run under the node security policy, the scanner container may fail with exit code `3` even though the connector, proxy, authentication, and SonarQube connectivity are working.
   - Diagnosis: If the scanner logs show that it connects to SonarQube and downloads the scanner engine or JRE before the JVM initialization error, treat this as a scanner runtime compatibility issue instead of a connector authentication issue.
   - Solution: Use the Java runtime already provided by the scanner image when it is compatible with your SonarQube version:
     ```bash
     ...
     ```
   - Verification: Run `java -version` in the scanner image and confirm that a supported Java runtime is available before disabling JRE provisioning.
...
1. **Check connector logs** (replace `<connectors-namespace>` with the
   namespace where the Connectors system is installed — defaults to
   `connectors-system` but is configurable):
   ```bash
   kubectl logs -n <connectors-namespace> -l app=connectors-proxy-service
   ```
