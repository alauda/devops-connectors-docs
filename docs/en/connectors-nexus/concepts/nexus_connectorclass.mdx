...
A single Nexus server usually fronts several package ecosystems — npm, Maven and PyPI — under different repository paths. Rather than creating one Connector per ecosystem (or mounting the per-format configuration files described below), you can declare a Nexus **aggregator** Connector that generates one child Connector per ecosystem: set `spec.generation.supportedClasses: [npm, maven, pypi]` on the Nexus ConnectorClass and list the ecosystems in the Nexus Connector's `spec.generators`. The controller materializes per-class `npm` / `maven` / `pypi` child Connectors that share the aggregator's host and credentials, and pipelines consume packages through those children.

This is the recommended way to serve multiple ecosystems from one Nexus server. For the full model — address templating, auth inheritance, the `GenerationReady` condition, recovery and lifecycle — see [Connector Generation](../../connectors/concepts/connector_generation.mdx).
...
**Optional parameter**: `registry` — the Nexus NPM proxy repository name (e.g., `npm-proxy`). Default: `""`. When empty, `.npmrc` is rendered with a placeholder comment instead of failing the CSI mount; supply this parameter to enable `npm install` / `npm publish`.
...
**Optional parameter**: `registry` — the Nexus NPM proxy repository name. Default: `""`. When empty, `.yarnrc.yml` is rendered with a placeholder comment instead of failing the CSI mount; supply this parameter to enable `yarn install` / `yarn npm publish`.
...
**Optional parameter**: `repository` — the Nexus PyPI proxy repository name (e.g., `pypi-proxy`). Default: `""`. When empty, `pip.conf` is rendered with a placeholder comment instead of failing the CSI mount; supply this parameter to enable `pip install`.
...
**Optional parameter**: `deployRepository` — the Nexus PyPI hosted repository name (e.g., `pypi-hosted`). Default: `""`. When empty, `.pypirc` is rendered with a placeholder comment instead of failing the CSI mount; supply this parameter to enable `twine upload`.
...
| `npmrc`       | `registry`  | No | `""` | Nexus NPM proxy repository name |
| `npmrc`       | `strictSSL` | No  | `"true"` | Whether to require SSL (`"true"` or `"false"`) |
| `yarnrc`      | `registry`  | No | `""` | Nexus NPM proxy repository name |
| `yarnrc`      | `strictSSL` | No  | `"true"` | Whether to require SSL (`"true"` or `"false"`) |
| `pipconf`     | `repository` | No | `""` | Nexus PyPI proxy repository name |
| `pypirc`      | `deployRepository` | No | `""` | Nexus PyPI hosted repository name for publishing |
